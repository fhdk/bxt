 ...
 */
class Iso8601TimePoint {
    static constexpr const auto time_format = "%Y-%m-%dT%H:%M:%SZ";

public:
    using ReflectionType = std::string;
    using time_point = std::chrono::system_clock::time_point;

    Iso8601TimePoint(const char* _str) {
        if (!_str) { throw std::invalid_argument("Cannot parse null string"); }
    ...
    Iso8601TimePoint(const std::string& _str)
        : Iso8601TimePoint(_str.c_str()) {}

    Iso8601TimePoint(const time_point& tp) : m_time_point(tp) {}

    ~Iso8601TimePoint() = default;

    static rfl::Result<Iso8601TimePoint>
        from_string(const char* _str) noexcept {
        try {
            return Iso8601TimePoint(_str);
        } catch (std::exception& e) { return rfl::Error(e.what()); }
    }

    static rfl::Result<Iso8601TimePoint> from_string(const std::string& _str) {
    ...
    time_point to_class() const { return m_time_point; }

    static Iso8601TimePoint from_class(const time_point& tp) { return {tp}; }
...
struct rfl::parsing::Parser<ReaderType,
                            WriterType,
