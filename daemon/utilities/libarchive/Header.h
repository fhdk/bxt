...
public:
    ...
    static Header default_file() {
        ...
        archive_entry_set_mtime(header,
    ...
    void operator=(const Header& rh) {
        m_entry = {archive_entry_clone(rh.m_entry.get()),
                   rh.m_entry.get_deleter()};
    }

    void operator=(Header&& rh) {
        m_entry = {std::move(rh.m_entry.release()),
                   std::move(rh.m_entry.get_deleter())};
    }
